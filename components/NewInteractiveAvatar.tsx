...
export default function NewInteractiveAvatar({
  ...
  async function startStreamingSession() {
    ...
    try {
      ...
      if (greet.trim()) {
        ...
        const greetingMsg: ChatMessage = { role: "assistant", content: greet };
  ...
  async function processUserMessage(userMessage: string) {
    ...
    try {
      const assistantReply = await continueConversation(updated);
      const newHistory = [
        ...updated,
        { role: "assistant", content: assistantReply } as ChatMessage,
      ];
      chatHistoryRef.current = newHistory;
      setChatHistory(newHistory);

      await sendText(assistantReply, "repeat");
    } catch (error: any) {
