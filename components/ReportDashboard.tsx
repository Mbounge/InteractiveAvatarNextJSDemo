...
import { Plus, Edit, Trash2, Loader2, Search, Clock, User, Users, Calendar, TrendingUp, UserCheck, Shield } from 'lucide-react';
...
interface ReportTypeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (type: 'skater' | 'goalie') => void;
}

const ReportTypeModal: React.FC<ReportTypeModalProps> = ({ isOpen, onClose, onSelect }) => {
  ...
  return (
    ...
    >
      <div 
        className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full transform transition-all duration-300 scale-100"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-2xl font-bold text-center bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent mb-2">
          Create New Report
        </h2>
        <p className="text-center text-gray-600 mb-8">
          Choose the type of player you are scouting.
        </p>
        <div className="flex flex-col sm:flex-row gap-4">
          <button
            onClick={() => onSelect('skater')}
            className="flex-1 flex flex-col items-center justify-center gap-3 p-6 bg-indigo-50 text-indigo-800 rounded-xl border-2 border-indigo-200 hover:border-indigo-500 hover:bg-indigo-100 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
            <UserCheck className="w-10 h-10" />
            <span className="font-semibold text-lg">Skater</span>
          </button>
          <button
            onClick={() => onSelect('goalie')}
            className="flex-1 flex flex-col items-center justify-center gap-3 p-6 bg-gray-50 text-gray-800 rounded-xl border-2 border-gray-200 hover:border-gray-500 hover:bg-gray-100 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
          >
            <Shield className="w-10 h-10" />
            <span className="font-semibold text-lg">Goalie</span>
          </button>
        </div>
      </div>
    </div>
...
interface ReportDashboardProps {
  accessCode: string;
  onSelectReport: (reportId: string | null, reportType?: 'skater' | 'goalie') => void;
}
...
const ReportDashboard: React.FC<ReportDashboardProps> = ({ accessCode, onSelectReport }) => {
  ...
  const fetchReports = async () => {
    ...
    try {
      const response = await fetch('/api/reports', {
        headers: { 'X-Scout-Identifier': accessCode, 'Cache-Control': 'no-cache', }
        
      });
      if (!response.ok) {
        throw new Error('Failed to fetch reports from the serverrr.');
      }
  ...
  const handleCreateReport = (type: 'skater' | 'goalie') => {
    setIsModalOpen(false);
    onSelectReport(null, type);
  };
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 auto-rows-fr">
                  {filteredReports.map((report) => {
                    ...
                    return (
                      <div
                        key={report._id}
                        className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-sm border border-white/50 hover:shadow-lg hover:scale-[1.02] transition-all duration-300 group overflow-hidden relative flex flex-col"
                      >
                        <div className={`absolute top-4 right-4 flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-bold ${
                          type === 'skater' ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-800'
                        }`}>
                          {type === 'skater' ? <UserCheck className="w-3.5 h-3.5" /> : <Shield className="w-3.5 h-3.5" />}
                          <span>{type === 'skater' ? 'Skater' : 'Goalie'}</span>
                        </div>
