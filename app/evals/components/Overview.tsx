...
const calculateToolStats = (transcript: Message[]) => {
  ...
  transcript.forEach((turn) => {
    if (turn.tool_calls) {
      const realTools = turn.tool_calls.filter(
        (tc: any) => tc.tool_name !== "end_call"
      );
...
const sortData = (data: any[], config: SortConfig) => {
  return [...data].sort((a, b) => {
    ...
    if (config.key === "abandonRate") {
      ...
      aVal =
        a.totalToolCalls > 0 ? a.successfulToolCalls / a.totalToolCalls : 0;
      bVal =
        b.totalToolCalls > 0 ? b.successfulToolCalls / b.totalToolCalls : 0;
...
export const Overview = ({
  ...
  const [agentSort, setAgentSort] = useState<SortConfig>({
    key: "count",
    direction: "desc",
  });
  const [modeSort, setModeSort] = useState<SortConfig>({
    key: "count",
    direction: "desc",
  });
  const [typeSort, setTypeSort] = useState<SortConfig>({
    key: "count",
    direction: "desc",
  });
  const [promptSort, setPromptSort] = useState<SortConfig>({
    key: "count",
    direction: "desc",
  });
  ...
  useEffect(() => {
    if (data.length > 0 && !dateRange.start) {
      const timestamps = data.map(
        (c) => c.metadata.start_time_unix_secs * 1000
      );
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(
      (c) =>
        c.metadata.start_time_unix_secs >= start &&
        c.metadata.start_time_unix_secs <= end
    );
  }, [data, dateRange]);
  ...
  const kpis = useMemo(() => {
    ...
    if (total === 0)
      return { total: 0, meaningful: 0, abandoned: 0, avgDuration: 0 };

    const meaningful = filteredData.filter(
      (c) =>
        c.metadata.call_duration_secs > 60 && getUserTurns(c.transcript) >= 5
    ).length;
    const abandoned = filteredData.filter(
      (c) => c.metadata.call_duration_secs < 15
    ).length;
    const avgDuration =
      filteredData.reduce((acc, c) => acc + c.metadata.call_duration_secs, 0) /
      total;
  ...
  const agentStats = useMemo(() => {
    ...
    filteredData.forEach((c) => {
      ...
      if (c.metadata.call_duration_secs > 60 && getUserTurns(c.transcript) >= 5)
        stats[id].deepCount++;
      const { usedTool, totalCalls, successCalls } = calculateToolStats(
        c.transcript
      );
  ...
  const modeStats = useMemo(() => {
    ...
    filteredData.forEach((c) => {
      ...
      const type =
        c.conversation_initiation_client_data?.dynamic_variables
          ?.context_type || "General";
      const key =
        type === "SystemPrompt" || type === "General" || type === "Unknown"
          ? "Standard Persona"
          : "Specialized Contexts";
      stats[key].count++;
  ...
  const chartData = useMemo(() => {
    ...
    filteredData.forEach((c) => {
      const date = format(
        new Date(c.metadata.start_time_unix_secs * 1000),
        "MMM dd"
      );
  ...
  const handleModeSort = (key: string) =>
    setModeSort((prev) => ({
      key,
      direction: prev.key === key && prev.direction === "desc" ? "asc" : "desc",
    }));
  const handleTypeSort = (key: string) =>
    setTypeSort((prev) => ({
      key,
      direction: prev.key === key && prev.direction === "desc" ? "asc" : "desc",
    }));
  const handlePromptSort = (key: string) =>
    setPromptSort((prev) => ({
      key,
      direction: prev.key === key && prev.direction === "desc" ? "asc" : "desc",
    }));
    ...
    <div className="space-y-8 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Real-time evaluation metrics. Click on names or colored metrics to
            filter the data.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <KpiCard
          ...
          value={`${kpis.total > 0 ? ((kpis.meaningful / kpis.total) * 100).toFixed(1) : 0}%`}
          subtext=">60s & 5+ User Turns"
        ...
        <KpiCard
          ...
          value={`${kpis.total > 0 ? ((kpis.abandoned / kpis.total) * 100).toFixed(1) : 0}%`}
          subtext="Duration < 15s"
          color="text-rose-600"
          bg="bg-rose-50"
        />
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="text-lg font-semibold text-[#161160] mb-6">
          Volume vs. Engagement Trend
        </h3>
        <div className="h-80 w-full">
          <ResponsiveContainer width="100%" height="100%" minWidth={0}>
            <LineChart
              ...
              onClick={(e) => {
                if (e && e.activeLabel) {
                  // Fix: Explicitly cast to string or wrap in String() to satisfy TypeScript
                  onDrillDown({ date: String(e.activeLabel) });
                }
              }}
          ...
          {sortData(agentStats, agentSort).map((agent: any) => {
            const funcSuccess =
              agent.totalToolCalls > 0
                ? (
                    (agent.successfulToolCalls / agent.totalToolCalls) *
                    100
                  ).toFixed(1)
                : "N/A";
            ...
            return (
              ...
              >
                <td className="px-6 py-4">
                  <button
                    onClick={(e) =>
                      handlePinRow(e, agent.id, `Stats: ${agent.name}`, agent, {
                        agentId: agent.id,
                        filterType: "All",
                      })
                    } // Pass Filter
                ...
                <td
                  className="px-6 py-4 font-medium text-indigo-700 cursor-pointer group-hover:text-indigo-900 group-hover:underline flex items-center gap-2"
                  onClick={() =>
                    handleDrillDownWithDate({
                      agentId: agent.id,
                      filterType: "All",
                    })
                  }
                ...
                <MetricCell
                  value={agent.count}
                  label={`${agent.name} Total`}
                  filter={{ agentId: agent.id, filterType: "All" }}
                  onDrillDown={handleDrillDownWithDate}
                />
                <MetricCell
                  value={`${((agent.abandonedCount / agent.count) * 100).toFixed(1)}%`}
                  label={`${agent.name} Abandon`}
                  filter={{ agentId: agent.id, filterType: "Short (<15s)" }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-rose-600"
                  bgColor="hover:bg-rose-50"
                />
                <MetricCell
                  value={`${((agent.deepCount / agent.count) * 100).toFixed(1)}%`}
                  label={`${agent.name} Deep`}
                  filter={{ agentId: agent.id, filterType: "Long (>60s)" }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-emerald-600"
                  bgColor="hover:bg-emerald-50"
                />
                <MetricCell
                  value={`${((agent.toolUseCount / agent.count) * 100).toFixed(1)}%`}
                  label={`${agent.name} Tool Use`}
                  filter={{ agentId: agent.id, filterType: "Tool Used" }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-indigo-600"
                  bgColor="hover:bg-indigo-50"
                />
                <MetricCell
                  value={funcSuccess === "N/A" ? "N/A" : `${funcSuccess}%`}
                  label={`${agent.name} Func Success`}
                  filter={{ agentId: agent.id, filterType: "Tool Failure" }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-indigo-600"
                  bgColor="hover:bg-indigo-50"
                />
                <td className="px-6 py-4 text-right font-mono text-slate-600">
                  {getMinAvgMax(agent.durations)}
                </td>
                <td className="px-6 py-4 text-right font-mono text-slate-600">
                  {getMinAvgMax(agent.turns)}
                </td>
              </tr>
            );
          })}
      ...
      <TableSection
        title="Interaction Mode: Standard vs. Specialized"
      ...
      >
        <thead className="text-xs text-slate-500 uppercase bg-slate-50">
          <tr>
            ...
            <SortableHeader
              label="Mode"
              sortKey="name"
              currentSort={modeSort}
              onSort={handleModeSort}
            />
            <SortableHeader
              label="Count"
              sortKey="count"
        ...
        <tbody className="divide-y divide-slate-100">
          {sortData(modeStats, modeSort).map((mode: any) => {
            ...
            const abandonRate = (
              (mode.abandonedCount / mode.count) *
              100
            ).toFixed(1);
            const deepRate = ((mode.deepCount / mode.count) * 100).toFixed(1);
            const funcSuccess =
              mode.totalToolCalls > 0
                ? (
                    (mode.successfulToolCalls / mode.totalToolCalls) *
                    100
                  ).toFixed(1)
                : "N/A";
            const modeKey = mode.name.includes("Standard")
              ? "Standard"
              : "Specialized";
            ...
            return (
              ...
              >
                  ...
                  <button
                    onClick={(e) =>
                      handlePinRow(
                        ...
                        `Stats: ${mode.name}`,
                        mode,
                        { interactionMode: modeKey, filterType: "All" }
                      )
                    } // Pass Filter
                    className={`p-1.5 rounded hover:bg-indigo-100 transition-colors ${pinned ? "text-indigo-600" : "text-slate-300 hover:text-indigo-600"}`}
                  >
                ...
                <MetricCell
                  value={mode.count}
                  label={`${mode.name} Total`}
                  filter={{ interactionMode: modeKey, filterType: "All" }}
                  onDrillDown={handleDrillDownWithDate}
                />
                <MetricCell
                  value={`${abandonRate}%`}
                  label={`${mode.name} Abandon`}
                  filter={{
                    interactionMode: modeKey,
                    filterType: "Short (<15s)",
                  }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-rose-600"
                  bgColor="hover:bg-rose-50"
                ...
                />
                <MetricCell
                  value={`${((mode.toolUseCount / mode.count) * 100).toFixed(1)}%`}
                  label={`${mode.name} Tool Use`}
                  filter={{ interactionMode: modeKey, filterType: "Tool Used" }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-indigo-600"
                  bgColor="hover:bg-indigo-50"
                />
                <MetricCell
                  value={funcSuccess === "N/A" ? "N/A" : `${funcSuccess}%`}
                  label={`${mode.name} Func Success`}
                  filter={{
                    interactionMode: modeKey,
                    filterType: "Tool Failure",
                  }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-indigo-600"
                  bgColor="hover:bg-indigo-50"
      ...
      <TableSection
        title="Context Prompt Types Performance"
      ...
      >
        ...
        <tbody className="divide-y divide-slate-100">
          {sortData(typeStats, typeSort).map((strat: any) => {
            const abandonRate = (
              (strat.abandonedCount / strat.count) *
              100
            ).toFixed(1);
            const deepRate = ((strat.deepCount / strat.count) * 100).toFixed(1);
            const funcSuccess =
              strat.totalToolCalls > 0
                ? (
                    (strat.successfulToolCalls / strat.totalToolCalls) *
                    100
                  ).toFixed(1)
                : "N/A";
            ...
            return (
              ...
              >
                  ...
                  <button
                    onClick={(e) =>
                      handlePinRow(
                        ...
                        `Stats: ${strat.type}`,
                        strat,
                        { contextType: strat.type, filterType: "All" }
                      )
                    } // Pass Filter
                    className={`p-1.5 rounded hover:bg-indigo-100 transition-colors ${pinned ? "text-indigo-600" : "text-slate-300 hover:text-indigo-600"}`}
                  >
                ...
                <MetricCell
                  value={strat.count}
                  label={`${strat.type} Total`}
                  filter={{ contextType: strat.type, filterType: "All" }}
                  onDrillDown={handleDrillDownWithDate}
                />
                <MetricCell
                  value={`${abandonRate}%`}
                  label={`${strat.type} Abandon`}
                  filter={{
                    contextType: strat.type,
                    filterType: "Short (<15s)",
                  }}
                ...
                />
                <MetricCell
                  value={`${((strat.toolUseCount / strat.count) * 100).toFixed(1)}%`}
                  label={`${strat.type} Tool Use`}
                  filter={{ contextType: strat.type, filterType: "Tool Used" }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-indigo-600"
                  bgColor="hover:bg-indigo-50"
                />
      ...
      <TableSection
        title="Specific Prompt Performance"
      ...
      >
        ...
        <tbody className="divide-y divide-slate-100">
          {sortData(promptStats, promptSort).map((prompt: any) => {
            const abandonRate = (
              (prompt.abandonedCount / prompt.count) *
              100
            ).toFixed(1);
            const deepRate = ((prompt.deepCount / prompt.count) * 100).toFixed(
              1
            );
            const funcSuccess =
              prompt.totalToolCalls > 0
                ? (
                    (prompt.successfulToolCalls / prompt.totalToolCalls) *
                    100
                  ).toFixed(1)
                : "N/A";
            ...
            return (
              ...
              >
                  ...
                  <button
                    onClick={(e) =>
                      handlePinRow(
                        ...
                        `Stats: ${prompt.title}`,
                        prompt,
                        { contextTitle: prompt.title, filterType: "All" }
                      )
                    } // Pass Filter
                    className={`p-1.5 rounded hover:bg-indigo-100 transition-colors ${pinned ? "text-indigo-600" : "text-slate-300 hover:text-indigo-600"}`}
                  >
                ...
                <MetricCell
                  value={prompt.count}
                  label={`${prompt.title} Total`}
                  filter={{ contextTitle: prompt.title, filterType: "All" }}
                  onDrillDown={handleDrillDownWithDate}
                />
                <MetricCell
                  value={`${abandonRate}%`}
                  label={`${prompt.title} Abandon`}
                  filter={{
                    contextTitle: prompt.title,
                    filterType: "Short (<15s)",
                  }}
                ...
                />
                <MetricCell
                  value={`${((prompt.toolUseCount / prompt.count) * 100).toFixed(1)}%`}
                  label={`${prompt.title} Tool Use`}
                  filter={{
                    contextTitle: prompt.title,
                    filterType: "Tool Used",
                  }}
                  onDrillDown={handleDrillDownWithDate}
                  textColor="text-indigo-600"
                  bgColor="hover:bg-indigo-50"
                ...
                <MetricCell
                  value={funcSuccess === "N/A" ? "N/A" : `${funcSuccess}%`}
                  label={`${prompt.title} Func Success`}
                  filter={{
                    contextTitle: prompt.title,
                    filterType: "Tool Failure",
                  }}
...
const KpiCard = ({
  ...
  <div
    className={`p-6 rounded-xl shadow-sm border border-slate-200 ${bg === "bg-white" ? "bg-white" : bg}`}
  >
